<template>
  <div class="min-h-screen bg-gray-950 text-gray-100 p-6">
    <div class="app-container py-6">
      ...
      <!-- Search bar -->
      <div class="rounded-2xl border border-gray-800 bg-gray-900/40 p-4 space-y-4">
        <form class="flex flex-col sm:flex-row gap-2 space-y-4" @submit.prevent="onSearch">
          <input
            v-model="query"
            class="w-full sm:flex-1 min-w-0 rounded-xl bg-gray-950 border border-gray-800 px-3 py-2 outline-none focus:border-gray-600"
            placeholder="Поиск"
          />

          <div class="flex gap-2 w-full sm:w-auto">
            <button
              class="w-full sm:w-auto rounded-xl bg-white text-gray-900 font-medium px-4 py-2 hover:bg-gray-100 disabled:opacity-60"
              type="submit"
            ...
            <button
              class="w-full sm:w-auto rounded-xl border border-gray-800 px-4 py-2 hover:bg-gray-900 disabled:opacity-60"
              type="button"
              @click="reset"
              :disabled="loading"
            >
      ...
      <div v-if="loading" class="space-y-3">
        <div
          v-for="i in 6"
          :key="i"
          class="rounded-2xl border border-gray-800 bg-gray-900/40 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.02)]"
        >
      ...
      <div v-if="!loading" class="flex flex-col gap-4">
        ...
        >
          <div class="flex items-start gap-3">
            <div
              class="w-11 h-11 rounded-2xl bg-gray-800 flex items-center justify-center text-sm font-semibold"
            >
              {{ initials(u.username) }}
            </div>
          ...
          <div class="flex flex-col items-end gap-2">
            <button
              v-if="me && u.id !== me.id"
              class="text-sm rounded-xl bg-white text-gray-900 font-medium px-3 py-1.5 hover:bg-gray-100 disabled:opacity-60 cursor-pointer"
              @click="message(u.id)"
        ...
        <div class="mt-6 text-xs text-gray-600">Показано: {{ people.length }}</div>
      </div>
    </div>
  </div>
</template>
